...
/** LLRPClient */
const rOSpec = new src_1.LLRPCore.ROSpec({
    data: {
        ROSpecID: 123,
        Priority: 0,
...
class Main {
    constructor(reader) {
        ...
        reader.on('error', err => { });
        reader.on('KEEPALIVE', () => {
            reader.send(new src_1.LLRPCore.KEEPALIVE_ACK());
        });
    }
    ...
    async scrubConfiguration() {
        ...
        if (rsp instanceof src_1.LLRPCore.DELETE_ROSPEC_RESPONSE) {
            console.log(`Delete all ROSpecs ${rsp.getLLRPStatus().getStatusCode()}`);
        }
        rsp = await this.resetToFactoryDefaults();
        if (rsp instanceof src_1.LLRPCore.SET_READER_CONFIG_RESPONSE) {
            console.log(`Reset to factory defaults ${rsp.getLLRPStatus().getStatusCode()}`);
        }
    }
    async addROSpec() {
        const rsp = await this.reader.transact(new src_1.LLRPCore.ADD_ROSPEC().setROSpec(rOSpec));
        if (rsp instanceof src_1.LLRPCore.ADD_ROSPEC_RESPONSE) {
            console.log(`Add ROSpec ${rsp.getLLRPStatus().getStatusCode()}`);
        }
    }
    async enableROSpec() {
        const rsp = await this.reader.transact(new src_1.LLRPCore.ENABLE_ROSPEC({
            data: {
                ROSpecID: rOSpec.getROSpecID()
            }
        }));
        if (rsp instanceof src_1.LLRPCore.ENABLE_ROSPEC_RESPONSE) {
            console.log(`Enable ROSpec ${rsp.getLLRPStatus().getStatusCode()}`);
        }
    }
    async startROSpec() {
        const rsp = await this.reader.transact(new src_1.LLRPCore.START_ROSPEC({
            data: {
                ROSpecID: rOSpec.getROSpecID()
            }
        }));
        if (rsp instanceof src_1.LLRPCore.START_ROSPEC_RESPONSE) {
            console.log(`Start ROSpec ${rsp.getLLRPStatus().getStatusCode()}`);
        }
    }
    async deleteAllROSpecs() {
        return this.reader.transact(new src_1.LLRPCore.DELETE_ROSPEC({
            data: {
                ROSpecID: 0 // all
            }
        }));
    ...
    async resetToFactoryDefaults() {
        return this.reader.transact(new src_1.LLRPCore.SET_READER_CONFIG({
            data: {
    ...
    async awaitAndPrintReport() {
        ...
        while (msg = await this.reader.recv(100000)) {
            if (msg instanceof src_1.LLRPCore.RO_ACCESS_REPORT) {
...
/** Main */
const main = new Main(new src_1.LLRPClient({
    host: "localhost",
    port: 5084
}));
main.run();

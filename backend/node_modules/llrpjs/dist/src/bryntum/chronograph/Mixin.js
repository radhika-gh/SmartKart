...
exports.MixinAny = exports.Mixin = exports.isInstanceOf = exports.ZeroBaseClass = exports.identity = void 0;
...
class MixinWalkDepthState {
    constructor() {
        ...
        this.linearizedByTopoLevelsSource = Iterator_js_1.MI(this.linearizedByTopoLevels());
    }
    static new(props) {
        const me = new this();
    ...
    buildElementsByTopoLevel() {
        let maxTopoLevel = 0;
        const baseElements = this.baseEl ? Iterator_js_1.CI(this.baseEl.walkDepthState.elementsByTopoLevel.values()).concat().toSet() : new Set();
        const map = Iterator_js_1.CI(this.sourceEl.requirements)
        ...
        return Iterator_js_1.CI(map).map(([level, elements]) => {
            return [level, Iterator_js_1.CI(elements).concat().uniqueOnly().filter(mixin => !baseElements.has(mixin)).sort((mixin1, mixin2) => mixin1.id - mixin2.id)];
        }).toMap();
    }
    *linearizedByTopoLevels() {
        yield* Iterator_js_1.CI(this.topoLevels).map(level => this.elementsByTopoLevel.get(level)).concat();
    }
}
...
exports.ZeroBaseClass = ZeroBaseClass;
//---------------------------------------------------------------------------------------------------------------------
class MixinState {
    constructor() {
        this.id = MIXIN_ID++;
        ...
        this.mixinLambda = exports.identity;
    ...
    buildMinimalClass() {
        ...
        const minimalClass = Object.assign(minimalClassConstructor, {
            ...
            derive: (base) => exports.Mixin([minimalClass, base], base => class extends base {
            }),
            $: this,
